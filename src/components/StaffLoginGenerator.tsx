...
export function StaffLoginGenerator({ staff, onComplete }: StaffLoginGeneratorProps) {
  const [passwords, setPasswords] = useState({ godown: '', big_shop: '', small_shop: '' });
  const [showPasswords, setShowPasswords] = useState({ godown: false, big_shop: false, small_shop: false });
    ...
    try {
      for (const member of activeStaff) {
        ...
        if (existing) {
          ...
          await supabase.from('app_users').update({ password_hash: hashedPassword, role, linked_staff_id: member.id } as any).eq('id', existing.id);
          generationResults.push({ name: member.name, email, status: 'exists' });
        } else {
          const hashedPassword = await hashPassword(password);
          const { error } = await supabase.from('app_users').insert({ email, password_hash: hashedPassword, role, linked_staff_id: member.id } as any);
          if (error) {
      ...
      <CardContent className="space-y-6">
          ...
          return (
            <div key={key} className="space-y-3 p-4 border border-border rounded-lg bg-muted/30">
              <div className="flex items-center justify-between">
                <Label className="font-semibold text-foreground">{label} Password</Label>
                <Badge variant="secondary" className="text-xs">{locationStaff.length} staff</Badge>
              </div>
              ...
              <div className="flex flex-wrap gap-1">
                {locationStaff.map(s => (
                  <Badge key={s.id} variant="outline" className="text-xs">{generateEmail(s.name, s.location)}</Badge>
                ))}
              </div>
            </div>
          );
        ...
        {results.length > 0 && (
          <div className="space-y-2 mt-4">
            <Label className="font-semibold text-foreground">Results</Label>
