...
interface DispatchHistoryFiltersProps {
...
export function DispatchHistoryFilters({ filters, onFiltersChange }: DispatchHistoryFiltersProps) {
  ...
  const clearFilters = () => {
    onFiltersChange({ itemType: 'all', source: 'all', destination: 'all' });
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Button
          ...
          className="text-xs gap-1.5 bg-white/90 border-gray-200"
        >
          <Filter className="h-3 w-3" />
          Filters
          {activeFilterCount > 0 && (
            <Badge className="bg-blue-600 text-white text-[10px] px-1.5 py-0 ml-1">{activeFilterCount}</Badge>
          )}
        </Button>
        {activeFilterCount > 0 && (
      ...
      {showFilters && (
        <div className="grid grid-cols-2 gap-3 p-3 bg-white/60 rounded-lg border border-gray-100">
          {/* Date From */}
